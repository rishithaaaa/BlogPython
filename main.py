...
# Without wraps, the decorated function
# would lose its original metadata, making debugging and introspection more difficult.
# Create admin-only decorator
def admin_only(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        #  the decorator proceeds to call the original function f with its original arguments (*args and **kwargs).
...
def show_post(post_id):
    if current_user.is_authenticated:
        ...
        comments_on_post = db.session.execute(db.select(Comment).where(Comment.post_id == post_id))
